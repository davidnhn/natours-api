...
const createSendToken = (user, statusCode, res) => {
  ...
  user.password = undefined; // remove le password de la reponse json
...
exports.restrictTo =
  ...
  (req, res, next) => {
    ...
    if (!roles.includes(req.user.role)) {
      return next(
        new AppError('You do not have permission to perform thuis action', 403)
      );
    }
    next();
  };
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  try {
    await sendEmail({
      ...
      subject: 'Your passowrd reset token (valid for 10min)',
      message,
    });
